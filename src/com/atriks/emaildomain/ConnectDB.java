...
public class ConnectDB {
    ...
    public static ArrayList<CompDom> retrieveCD() throws SQLException {
        ...
        ResultSet rs = s.executeQuery("select top 10000 dbo.RemoveNonASCII(company) as company, ehost, total from qa_li_company_email_domains (nolock) where status is null and parsed = 0 order by total desc");
        while (rs.next()) {
            String c = rs.getString("company");
            String d = rs.getString("ehost");
            PreparedStatement ps = con.prepareStatement("select ehost from qa_li_company_email_domains (nolock) where company = '" + c.replace("'", "''") + "' and status is null");
